...
import (
	...
	"github.com/ccmchain/go-ccmchain/ccm/fetcher"
...
type ProtocolManager struct {
	...
	fastSync  uint32 // Flag whccmer fast sync is enabled (gets disabled if we already have blocks)
	acceptTxs uint32 // Flag whccmer we're considered synchronised (enables transaction processing)
...
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb ccmdb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		// out a way yet where nodes can decide unilaterally whccmer the network is new
...
// handle is the callback invoked to manage the life cycle of an ccm peer. When
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= ccm63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= ccm63 && msg.Code == NodeDataMsg:
	...
	case p.version >= ccm63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= ccm63 && msg.Code == ReceiptsMsg:
