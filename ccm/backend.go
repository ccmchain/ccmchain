...
import (
	...
	"github.com/ccmchain/go-ccmchain/p2p/enr"
...
type Ethereum struct {
	...
	// DB interfaces
	chainDb ccmdb.Database // Block chain database
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	ccm.blockchain, err = core.NewBlockChain(chainDb, cacheConfig, chainConfig, ccm.engine, vmConfig, ccm.shouldPreserve)
	if err != nil {
		return nil, err
	}
	...
	if compat, ok := genesisErr.(*params.ConfigCompatError); ok {
		...
		ccm.blockchain.SetHead(compat.RewindTo)
		rawdb.WriteChainConfig(chainDb, genesisHash, chainConfig)
	}
	ccm.bloomIndexer.Start(ccm.blockchain)

	if config.TxPool.Journal != "" {
		config.TxPool.Journal = ctx.ResolvePath(config.TxPool.Journal)
	}
	ccm.txPool = core.NewTxPool(config.TxPool, chainConfig, ccm.blockchain)
	...
	if ccm.protocolManager, err = NewProtocolManager(chainConfig, checkpoint, config.SyncMode, config.NetworkId, ccm.eventMux, ccm.txPool, ccm.engine, ccm.blockchain, chainDb, cacheLimit, config.Whitelist); err != nil {
		return nil, err
	}
	ccm.miner = miner.New(ccm, &config.Miner, chainConfig, ccm.EventMux(), ccm.engine, ccm.isLocalBlock)
	ccm.miner.SetExtra(makeExtraData(config.Miner.ExtraData))

	ccm.APIBackend = &EthAPIBackend{ctx.ExtRPCEnabled(), ccm, nil}
	gpoParams := config.GPO
	if gpoParams.Default == nil {
		gpoParams.Default = config.Miner.GasPrice
	}
	ccm.APIBackend.gpo = gasprice.NewOracle(ccm.APIBackend, gpoParams)
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *ccmash.Config, notify []string, noverify bool, db ccmdb.Database) consensus.Engine {
	...
	case ccmash.ModeFake:
		log.Warn("Ethash used in fake mode")
		return ccmash.NewFaker()
	case ccmash.ModeTest:
		log.Warn("Ethash used in test mode")
		return ccmash.NewTester(nil, noverify)
	case ccmash.ModeShared:
		log.Warn("Ethash used in shared mode")
		return ccmash.NewShared()
	default:
...
func (s *Ethereum) Etherbase() (eb common.Address, err error) {
	...
	if ccmerbase != (common.Address{}) {
		return ccmerbase, nil
	}
	if wallets := s.AccountManager().Wallets(); len(wallets) > 0 {
		if accounts := wallets[0].Accounts(); len(accounts) > 0 {
			ccmerbase := accounts[0].Address
	...
	return common.Address{}, fmt.Errorf("ccmerbase must be explicitly specified")
}

// isLocalBlock checks whccmer the specified block is mined
...
func (s *Ethereum) isLocalBlock(block *types.Block) bool {
	...
	// Check whccmer the given address is ccmerbase.
	s.lock.RLock()
	ccmerbase := s.ccmerbase
	s.lock.RUnlock()
	if author == ccmerbase {
	...
	// Check whccmer the given address is specified by `txpool.local`
	// CLI flag.
...
func (s *Ethereum) Start(srvr *p2p.Server) error {
	...
	s.netRPCService = ccmapi.NewPublicNetAPI(srvr, s.NetVersion())
