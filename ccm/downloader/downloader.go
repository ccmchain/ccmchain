...
import (
	...
	"github.com/ccmchain/go-ccmchain"
...
var (
	...
	maxQueuedHeaders         = 32 * 1024                    // [ccm/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB    ccmdb.Database  // Database to state sync into (and deduplicate via)
	...
	// Channels
	headerCh      chan dataPack        // [ccm/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [ccm/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [ccm/63] Channel receiving inbound receipts
	bodyWakeCh    chan bool            // [ccm/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [ccm/63] Channel to signal the receipt fetcher of new tasks
	headerProcCh  chan []*types.Header // [ccm/62] Channel to feed the header processor new tasks

	// for stateFetcher
	stateSyncStart chan *stateSync
	trackStateReq  chan *stateReq
	stateCh        chan dataPack // [ccm/63] Channel receiving inbound node state data
...
func New(checkpoint uint64, stateDb ccmdb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// Synchronising returns whccmer the downloader is currently retrieving blocks.
...
func (d *Downloader) fetchHeight(p *peerConnection) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somccming valid
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somccming completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somccming)
				// R: Nothing to give
				if d.mode != LightSync {
				...
				// peer gave us somccming useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if somccming failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCanceled
				default:
