...
import (
	...
	"github.com/ccmchain/go-ccmchain/common/prque"
...
type fetchRequest struct {
	...
	From    uint64          // [ccm/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [ccm/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead      common.Hash                    // [ccm/62] Hash of the last queued header to verify order
	headerTaskPool  map[uint64]*types.Header       // [ccm/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [ccm/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [ccm/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [ccm/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [ccm/62] Result cache accumulating the completed headers
	headerProced    int                            // [ccm/62] Number of headers already processed from the results
	headerOffset    uint64                         // [ccm/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [ccm/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [ccm/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [ccm/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [ccm/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [ccm/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [ccm/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [ccm/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [ccm/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [ccm/63] Set of the completed receipt fetches
...
// InFlightHeaders retrieves whccmer there are header fetch requests currently
// in flight.
...
// InFlightBlocks retrieves whccmer there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whccmer there are receipt fetch requests currently
// in flight.
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading somccming (sanity check to
	// not corrupt state)
...
// returns a flag whccmer empty blocks were queued requiring processing.
...
// also returns a flag whccmer empty receipts were queued requiring importing.
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading somccming (sanity check not to corrupt state)
	if taskQueue.Empty() {
