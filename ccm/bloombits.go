...
func (ccm *Ethereum) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				...
				case request := <-ccm.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(ccm.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(ccm.chainDb, task.Bit, section, head); err == nil {
...
type BloomIndexer struct {
	...
	db      ccmdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db ccmdb.Database, size, confirms uint64) *core.ChainIndexer {
