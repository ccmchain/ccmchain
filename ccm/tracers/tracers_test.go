...
var makeTest = function(tx, rewind) {
  ...
  var block   = ccm.getBlock(ccm.getTransaction(tx).blockHash);
  var genesis = ccm.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.ccm.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  ccm.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
