...
type ccmstatsConfig struct {
	URL string `toml:",omitempty"`
}

type gccmConfig struct {
	Eth       ccm.Config
	Shh       whisper.Config
	Node      node.Config
	Ethstats  ccmstatsConfig
	Dashboard dashboard.Config
}

func loadConfig(file string, cfg *gccmConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gccmConfig) {
	// Load defaults.
	cfg := gccmConfig{
		Eth:       ccm.DefaultConfig,
