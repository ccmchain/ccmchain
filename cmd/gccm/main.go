...
var (
	...
	app = utils.NewApp(gitCommit, gitDate, "the go-ccmchain command line interface")
...
func init() {
	// Initialize the CLI app and start Gccm
	app.Action = gccm
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2019 The go-ccmchain Authors"
...
// gccm is the main entry point into the system if no special subcommand is ran.
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	ccmClient := ccmclient.NewClient(rpcClient)
	...
	if ctx.GlobalInt(utils.LightLegacyServFlag.Name) > 0 || ctx.GlobalInt(utils.LightServeFlag.Name) > 0 {
		...
		ccmService.SetContractBackend(ccmClient)
	}
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
		...
		lesService.SetContractBackend(ccmClient)
	}
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, ccmClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		ccmchain.TxPool().SetGasPrice(gasprice)

		threads := ctx.GlobalInt(utils.MinerLegacyThreadsFlag.Name)
		if ctx.GlobalIsSet(utils.MinerThreadsFlag.Name) {
			threads = ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		}
		if err := ccmchain.StartMining(threads); err != nil {
