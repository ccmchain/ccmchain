...
FROM ccmchain/client-go:alltools-latest

ADD genesis.json /genesis.json
ADD account.json /account.json
ADD account.pass /account.pass

EXPOSE 8080 17575 17575/udp

ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--ccmstats", "{{.Ethstats}}", "--ccmport", "{{.EthPort}}",     \
...
// checkFaucet does a health-check against a faucet server to verify whccmer
