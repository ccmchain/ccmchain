...
func (w *wizard) makeGenesis() {
	...
	switch {
	case choice == "1":
		// In case of ccmash, we're pretty much done
...
// importGenesis imports a Gccm genesis spec into puppccm.
...
func (w *wizard) manageGenesis() {
	// Figure out whccmer to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alccm.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppccm and Gccm
		gccmJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile((gccmJson), out, 0644); err != nil {
		...
		if spec, err := newAlccmGenesisSpec(w.network, w.conf.Genesis); err != nil {
			...
			saveGenesis(folder, w.network, "alccm", spec)
		}
	...
	case "3":
		...
		w.conf.flush()
	default:
		log.Error("That's not somccming I can do")
		return
	}
}
