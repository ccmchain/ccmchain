...
import (
	...
	math2 "github.com/ccmchain/go-ccmchain/common/math"
...
// alccmGenesisSpec represents the genesis specification format used by the
// C++ Ethereum implementation.
type alccmGenesisSpec struct {
	...
	Accounts map[common.UnprefixedAddress]*alccmGenesisSpecAccount `json:"accounts"`
}

// alccmGenesisSpecAccount is the prefunded genesis account and/or precompiled
// contract definition.
type alccmGenesisSpecAccount struct {
	Balance     *math2.HexOrDecimal256   `json:"balance"`
	Nonce       uint64                   `json:"nonce,omitempty"`
	Precompiled *alccmGenesisSpecBuiltin `json:"precompiled,omitempty"`
}

// alccmGenesisSpecBuiltin is the precompiled contract definition.
type alccmGenesisSpecBuiltin struct {
	Name          string                         `json:"name,omitempty"`
	StartingBlock hexutil.Uint64                 `json:"startingBlock,omitempty"`
	Linear        *alccmGenesisSpecLinearPricing `json:"linear,omitempty"`
}

type alccmGenesisSpecLinearPricing struct {
	Base uint64 `json:"base"`
	Word uint64 `json:"word"`
}

// newAlccmGenesisSpec converts a go-ccmchain genesis block into a Alccm-specific
// chain specification format.
func newAlccmGenesisSpec(network string, genesis *core.Genesis) (*alccmGenesisSpec, error) {
	// Only ccmash is currently supported between go-ccmchain and alccm
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Alccm format
	spec := &alccmGenesisSpec{
		SealEngine: "Ethash",
	}
	// Some defaults
	...
	spec.Params.BlockReward = (*hexutil.Big)(ccmash.FrontierBlockReward)
	...
	spec.setPrecompile(1, &alccmGenesisSpecBuiltin{Name: "ecrecover",
		Linear: &alccmGenesisSpecLinearPricing{Base: 3000}})
	spec.setPrecompile(2, &alccmGenesisSpecBuiltin{Name: "sha256",
		Linear: &alccmGenesisSpecLinearPricing{Base: 60, Word: 12}})
	spec.setPrecompile(3, &alccmGenesisSpecBuiltin{Name: "ripemd160",
		Linear: &alccmGenesisSpecLinearPricing{Base: 600, Word: 120}})
	spec.setPrecompile(4, &alccmGenesisSpecBuiltin{Name: "identity",
		Linear: &alccmGenesisSpecLinearPricing{Base: 15, Word: 3}})
	if genesis.Config.ByzantiumBlock != nil {
		spec.setPrecompile(5, &alccmGenesisSpecBuiltin{Name: "modexp",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
		spec.setPrecompile(6, &alccmGenesisSpecBuiltin{Name: "alt_bn128_G1_add",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alccmGenesisSpecLinearPricing{Base: 500}})
		spec.setPrecompile(7, &alccmGenesisSpecBuiltin{Name: "alt_bn128_G1_mul",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64()),
			Linear:        &alccmGenesisSpecLinearPricing{Base: 40000}})
		spec.setPrecompile(8, &alccmGenesisSpecBuiltin{Name: "alt_bn128_pairing_product",
			StartingBlock: (hexutil.Uint64)(genesis.Config.ByzantiumBlock.Uint64())})
	}
	return spec, nil
}

func (spec *alccmGenesisSpec) setPrecompile(address byte, data *alccmGenesisSpecBuiltin) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alccmGenesisSpecAccount)
	}
	addr := common.UnprefixedAddress(common.BytesToAddress([]byte{address}))
	if _, exist := spec.Accounts[addr]; !exist {
		spec.Accounts[addr] = &alccmGenesisSpecAccount{}
	}
	spec.Accounts[addr].Precompiled = data
}

func (spec *alccmGenesisSpec) setAccount(address common.Address, account core.GenesisAccount) {
	if spec.Accounts == nil {
		spec.Accounts = make(map[common.UnprefixedAddress]*alccmGenesisSpecAccount)
	}
...
func (spec *alccmGenesisSpec) setByzantium(num *big.Int) {
	spec.Params.ByzantiumForkBlock = hexutil.Uint64(num.Uint64())
}

func (spec *alccmGenesisSpec) setConstantinople(num *big.Int) {
...
// newParityChainSpec converts a go-ccmchain genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ccmash is currently supported between go-ccmchain and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
...
func (spec *parityChainSpec) setByzantium(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(ccmash.ByzantiumBlockReward)
...
func (spec *parityChainSpec) setConstantinople(num *big.Int) {
	spec.Engine.Ethash.Params.BlockReward[hexutil.EncodeBig(num)] = hexutil.EncodeBig(ccmash.ConstantinopleBlockReward)
...
// newPyEthereumGenesisSpec converts a go-ccmchain genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only ccmash is currently supported between go-ccmchain and pyccmchain
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
