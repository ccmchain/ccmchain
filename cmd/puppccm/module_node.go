...
// nodeDockerfile is the Dockerfile required to run an Ccmchain node.
var nodeDockerfile = `
FROM ccmchain/client-go:latest
...
// an Ccmchain node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.ccmchain{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.ccmash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new Ccmchain node container to a remote machine via SSH,
// docker and docker-compose. If an instance with the specified network name
// already exists there, it will be overwritten!
func deployNode(client *sshClient, network string, bootnodes []string, config *nodeInfos, nocache bool) ([]byte, error) {
	kind := "sealnode"
	if config.keyJSON == "" && config.ccmerbase == "" {
		kind = "bootnode"
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.ccmerbase != "" {
			...
			report["Ethash directory"] = info.ccmashdir
			report["Miner account"] = info.ccmerbase
		}
		if info.keyJSON != "" {
...
// whccmer it's running, and if yes, whccmer it's responsive.
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gccm --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		...
		ccmashdir:  infos.volumes["/root/.ccmash"],
		port:       port,
		peersTotal: totalPeers,
		peersLight: lightPeers,
		ccmstats:   infos.envvars["STATS_NAME"],
		ccmerbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasLimit:   gasLimit,
