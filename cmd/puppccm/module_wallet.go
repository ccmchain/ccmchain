...
FROM puppccm/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gccm --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gccm --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ccmstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppccmNetworkID/{{.NetworkID}}/g' dist/js/ccmerwallet-master.js && \
	sed -i 's/PuppccmNetwork/{{.Network}}/g'     dist/js/ccmerwallet-master.js && \
	sed -i 's/PuppccmDenom/{{.Denom}}/g'         dist/js/ccmerwallet-master.js && \
	sed -i 's/PuppccmHost/{{.Host}}/g'           dist/js/ccmerwallet-master.js && \
	sed -i 's/PuppccmRPCPort/{{.RPCPort}}/g'     dist/js/ccmerwallet-master.js
...
func (info *walletInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":      info.ccmstats,
...
// checkWallet does a health-check against web wallet server to verify whccmer
// it's running, and if yes, whccmer it's responsive.
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		datadir:  infos.volumes["/root/.ccmchain"],
		nodePort: nodePort,
		rpcPort:  rpcPort,
		webHost:  host,
		webPort:  webPort,
		ccmstats: infos.envvars["STATS"],
	}
	return stats, nil
}
