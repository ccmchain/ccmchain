...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.Genesis.Config.Ethash != nil && !boot {
		fmt.Println()
		if infos.ccmashdir == "" {
			fmt.Printf("Where should the ccmash mining DAGs be stored on the remote machine?\n")
			infos.ccmashdir = w.readString()
		} else {
			fmt.Printf("Where should the ccmash mining DAGs be stored on the remote machine? (default = %s)\n", infos.ccmashdir)
			infos.ccmashdir = w.readDefaultString(infos.ccmashdir)
		}
	}
	...
	if !boot {
		if w.conf.Genesis.Config.Ethash != nil {
			// Ethash based miners only need an ccmerbase to mine against
			fmt.Println()
			if infos.ccmerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.ccmerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.ccmerbase)
				infos.ccmerbase = w.readDefaultAddress(common.HexToAddress(infos.ccmerbase)).Hex()
			}
