...
FROM puppccm/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gccm --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gccm --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --ccmstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcapi "net,web3,ccm,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gccm --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.EthPort}} --bootnodes {{.Bootnodes}} --ccmstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcapi "net,web3,ccm,shh,debug" --rpccorsdomain "*" --rpcvhosts "*" --ws --wsorigins "*" &' >> explorer.sh && \
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"EthPort":     config.node.port,
		"EthName":     config.node.ccmstats[:strings.Index(config.node.ccmstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Ethstats username":       info.node.ccmstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			...
			ccmstats: infos.envvars["ETH_NAME"],
		},
