...
import (
	...
	"github.com/ccmchain/go-ccmchain/p2p/simulations"
	"github.com/ccmchain/go-ccmchain/p2p/simulations/adapters"
	...
	app.Commands = []cli.Command{
		...
		},
		{
			...
			Subcommands: []cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <mccmod> [<args>]",
					Usage:     "call a node RPC mccmod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						cli.BoolFlag{
							Name:  "subscribe",
							Usage: "mccmod is a subscription",
						},
					},
				},
			},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool("subscribe") {
		return rpcSubscribe(rpcClient, ctx.App.Writer, mccmod, args[3:]...)
	}
	...
	if err := rpcClient.Call(&result, mccmod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, mccmod string, args ...string) error {
	parts := strings.SplitN(mccmod, "_", 2)
	namespace := parts[0]
	mccmod = parts[1]
