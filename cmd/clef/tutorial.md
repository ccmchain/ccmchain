...
```text
$ clef --keystore ~/.ccmchain/rinkeby/keystore --chainid 4

INFO [07-01|11:00:46.385] Starting signer                          chainid=4 keystore=$HOME/.ccmchain/rinkeby/keystore light-kdf=false advanced=false
...
```text
echo '{"id": 1, "jsonrpc": "2.0", "mccmod": "account_list"}' | nc -U ~/.clef/clef.ipc
```
...
You can select which accounts the caller can see
  [x] 0xD9C9Cd5f6779558b6e0eD4e6Acf6b1947E7fA1F3
    URL: keystore://$HOME/.ccmchain/rinkeby/keystore/UTC--2017-04-14T15-15-00.327614556Z--d9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3
  [x] 0x086278A6C067775F71d6B2BB1856Db6E28c30418
    URL: keystore://$HOME/.ccmchain/rinkeby/keystore/UTC--2018-02-06T22-53-11.211657239Z--086278a6c067775f71d6b2bb1856db6e28c30418
-------------------------------------------
Request context:
	NA -> NA -> NA
...
Depending on whccmer we approve or deny the request, the original NetCat process will get:
...
Apart from listing accounts, you can also *request* creating a new account; signing transactions and data; and recovering signatures. You can find the available mccmods in the Clef [External API Spec](https://github.com/ccmchain/go-ccmchain/tree/master/cmd/clef#external-api-1) and the [External API Changelog](https://github.com/ccmchain/go-ccmchain/blob/master/cmd/clef/extapi_changelog.md).

*Note, the number of things you can do from the External API is deliberately small, since we want to limit the power of remote calls by as much as possible! Clef has an [Internal API](https://github.com/ccmchain/go-ccmchain/tree/master/cmd/clef#ui-api-1) too for the UI (User Interface) which is much richer and can support custom interfaces on top. But that's out of scope here.*
...
```text
$ clef --keystore ~/.ccmchain/rinkeby/keystore --chainid 4 --rules rules.js

INFO [07-01|13:39:49.726] Rule engine configured                   file=rules.js
INFO [07-01|13:39:49.726] Starting signer                          chainid=4 keystore=$HOME/.ccmchain/rinkeby/keystore light-kdf=false advanced=false
...
*Note, to make this example work, please use you own accounts. You can create a new account either via Clef or the traditional account CLI tools. If the latter was chosen, make sure both Clef and Gccm use the same keystore by specifying `--keystore path/to/your/keystore` when running Clef.*
...
INFO [07-01|14:12:41.636] Starting signer                          chainid=4 keystore=$HOME/.ccmchain/rinkeby/keystore light-kdf=false advanced=false
...
```
$ echo '{"id": 1, "jsonrpc":"2.0", "mccmod":"account_signData", "params":["data/plain", "0xd9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3", "0x202062617a6f6e6b2062617a2067617a0a"]}' | nc -U ~/.clef/clef.ipc
...
$ echo '{"id": 1, "jsonrpc":"2.0", "mccmod":"account_signData", "params":["data/plain", "0xd9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3", "0x2020626f6e6b2062617a2067617a0a"]}' | nc -U ~/.clef/clef.ipc
{"jsonrpc":"2.0","id":1,"error":{"code":-32000,"message":"Request denied"}}
```
...
For more details on writing automatic rules, please see the [rules spec](https://github.com/ccmchain/go-ccmchain/blob/master/cmd/clef/rules.md).
...
We can try this by running Clef with our previous rules on Rinkeby (for now it's a good idea to allow auto-listing accounts, since Gccm likes to retrieve them once in a while).

```text
$ clef --keystore ~/.ccmchain/rinkeby/keystore --chainid 4 --rules rules.js
```

In a different window we can start Gccm, list our accounts, even list our wallets to see where the accounts originate from:

```text
$ gccm --rinkeby --signer=~/.clef/clef.ipc console
...
> ccm.sendTransaction({from: ccm.accounts[0], to: ccm.accounts[0]})
```
...
*Note, if you enable the external signer backend in Gccm, all other account management is disabled. This is because long term we want to remove account management from Gccm.*
