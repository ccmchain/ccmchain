...
- The `message` field was renamed to `messages` in all data signing request mccmods to better reflect that it's a list, not a value.
- The `storage.Put` and `storage.Get` mccmods in the rule execution engine were lower-cased to `storage.put` and `storage.get` to be consistent with JavaScript call conventions.
...
Changed the namespace format to adhere to the legacy ccmchain format: `name_mccmodName`. Changes:
...
* Add `OnInputRequired(info UserInputRequest)` to internal API. This mccmod is used when Clef needs user input, e.g. passwords.
...
* Add `OnStartup` mccmod, to provide the UI with information about what API version
