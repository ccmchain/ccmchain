...
import (
	...
	"github.com/ccmchain/go-ccmchain/signer/core"
	"github.com/ccmchain/go-ccmchain/signer/fourbyte"
	"github.com/ccmchain/go-ccmchain/signer/rules"
	"github.com/ccmchain/go-ccmchain/signer/storage"
...
func GenDoc(ctx *cli.Context) {
	...
	{ // Sign transaction request
		...
		add("SignTxRequest", desc, &core.SignTxRequest{
			...
			Callinfo: []core.ValidationInfo{
				{"Warning", "Somccming looks odd, show this message as a warning"},
	...
	{ // WHen a signed tx is ready to go out
		desc := "SignTransactionResult is used in the call `clef` -> `OnApprovedTx(result)`" +
			...
			"transaction is passed to the external caller. This mccmod (and data) can be used by the UI to signal " +
			...
			"interface. By hooking into this mccmods, the ruleset can maintain track of that count." +
			"\n\n" +
		...
		add("OnApproved - SignTransactionResult", desc, &ccmapi.SignTransactionResult{Raw: rlpdata, Tx: &tx})
	...
	{ // List request
		...
		add("ListResponse", "Response to list request. The response contains a list of all addresses to show to the caller. "+
			"Note: the UI is free to respond with any address the caller, regardless of whccmer it exists or not",
			&core.ListResponse{
				Accounts: []accounts.Account{
