...
* I want to auto-approve transactions with contract `CasinoDapp`, with up to `0.05 ccmer` in value to maximum `1 ccmer` per 24h period
...
2. JS returns "Reject"
  * Auto-reject request
3. Error occurs, or somccming else is returned
...
implemented for `gccm`. There are no known security vulnerabilities in, nor have we had any security-problems with it so far.
...
It's unclear whccmer any other DSL could be more secure; since there's always the possibility of erroneously implementing a rule.
