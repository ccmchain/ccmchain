...
func (ui *headlessUi) OnApprovedTx(tx ccmapi.SignTransactionResult) {}
...
func tmpDirName(t *testing.T) string {
	d, err := ioutil.TempDir("", "ccm-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *ccmapi.SignTransactionResult
		err       error
	)
	...
	res, err = api.SignTransaction(context.Background(), tx, &mccmodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &mccmodSig)
	if err != nil {
		t.Fatal(err)
	}
