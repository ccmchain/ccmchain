...
// NotFound is returned by API mccmods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mccmods in this interface access raw
...
type ChainReader interface {
	...
	// This mccmod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mccmod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mccmod to
...
// TransactionSender wraps transaction sending. The SendTransaction mccmod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mccmod can be used to retrieve the
