...
// Reference: https://github.com/ccmchain/wiki/wiki/Ethash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see ccmash_io_prepare
enum ccmash_io_rc {
...
/**
 * Logs a critical error in important parts of ccmash. Should mostly help
...
/**
 * Prepares io for ccmash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the ccmash
 ...
 *                           https://github.com/ccmchain/wiki/wiki/Ethash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum ccmash_io_rc
 */
 ...
 */
FILE* ccmash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* ccmash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool ccmash_mkdir(char const* dirname);
 ...
 */
bool ccmash_file_size(FILE* f, size_t* ret_size);
 ...
 */
char* ccmash_io_create_filename(
	char const* dirname,
	char const* filename,
 ...
 *
 * The spec defining this directory is here: https://github.com/ccmchain/wiki/wiki/Ethash-DAG
 *
 ...
 */
bool ccmash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool ccmash_io_mutable_name(
	uint32_t revision,
