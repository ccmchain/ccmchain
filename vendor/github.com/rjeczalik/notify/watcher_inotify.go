...
// goroutines which construct valid notify events. This mccmod uses
...
// all file descriptors opened by lazyinit mccmod and returns afterwards.
...
// It is run in a separate goroutine in order to not block loop mccmod when
...
// inotify map. This mccmod may also split one raw event into two different ones
...
// decode uses internally stored mask to distinguish whccmer system-independent
// or system-dependent event is requested. The first one is created by modifying
// `e` argument. decode mccmod sets e.event value to 0 when an event should be
...
// This mccmod is allowed to return EINVAL error when concurrently requested to
