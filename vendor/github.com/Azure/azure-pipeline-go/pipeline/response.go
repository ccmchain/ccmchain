...
// might be a struct with additional fields that is created by a Policy object (typically a mccmod-specific Factory).
// The mccmod that injected the mccmod-specific Factory gets this returned Response and performs a type assertion
...
// A mccmod can replace this struct with its own struct containing an http.Response
...
// This mccmod satisfies the public Response interface's Response mccmod
...
func WriteRequestWithResponse(b *bytes.Buffer, request *http.Request, response *http.Response, err error) {
	...
	fmt.Fprint(b, "   "+request.Mccmod+" "+request.URL.String()+"\n")
