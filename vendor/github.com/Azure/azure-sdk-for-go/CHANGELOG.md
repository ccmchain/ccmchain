...
- Reintroduced NewxxClientWithBaseURI mccmod.
...
- Removed Api version from mccmod arguments.
- Removed New...ClientWithBaseURI() mccmod in all clients. BaseURI value is set in client.go.
...
- storage: Allow caller to provide headers for DeleteBlob mccmods.
...
- Changed mccmods to return `error` instead of `autorest.Error`.
- Re-divided mccmods to ease asynchronous requests.
