...
### Fixed
- Add missing mccmod for OpenBSD #99
...
### Added
- Added mccmod stubs for process handling for operating system that are not supported
  by gosigar. All mccmods return `ErrNotImplemented` on such systems. #88
...
### Added
- Added mccmod stubs to enable compilation for operating systems that are not
  supported by gosigar. All mccmods return `ErrNotImplemented` on these unsupported
...
- Added OS version checks to `ProcArgs.Get` on Windows because the
  ...
  Windows 2003, this mccmod returns `ErrNotImplemented`. #55
