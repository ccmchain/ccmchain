...
import (
	...
	"github.com/ccmchain/go-ccmchain/accounts/usbwallet"
	"github.com/ccmchain/go-ccmchain/log"
	"github.com/ccmchain/go-ccmchain/rpc"
	"github.com/robertkrimen/otto"
)

// bridge is a collection of JavaScript utility mccmods to bride the .js runtime
// environment and the Go RPC connection backing the remote mccmod calls.
type bridge struct {
...
// NewAccount is a wrapper around the personal.newAccount RPC mccmod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC mccmod (saved in jccm.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call otto.FunctionCall) (response otto.Value) {
	...
	ret, err := call.Otto.Call("jccm.newAccount", nil, password)
	if err != nil {
		throwJSException(err.Error())
	}
	return ret
...
func (b *bridge) OpenWallet(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jccm.openWallet", nil, wallet, passwd)
	if err == nil {
		return val
	}
	...
	case strings.HasSuffix(err.Error(), scwallet.ErrPairingPasswordNeeded.Error()):
		...
		if val, err = call.Otto.Call("jccm.openWallet", nil, wallet, passwd); err != nil {
...
func (b *bridge) readPassphraseAndReopenWallet(call otto.FunctionCall) (otto.Value, error) {
	...
	return call.Otto.Call("jccm.openWallet", nil, wallet, passwd)
}
...
// UnlockAccount is a wrapper around the personal.unlockAccount RPC mccmod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC mccmod (saved in jccm.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call otto.FunctionCall) (response otto.Value) {
	...
	val, err := call.Otto.Call("jccm.unlockAccount", nil, account, passwd, duration)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
// Sign is a wrapper around the personal.sign RPC mccmod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC mccmod (saved in
// jccm.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call otto.FunctionCall) (response otto.Value) {
	...
	// Send the request to the backend and return
	val, err := call.Otto.Call("jccm.sign", nil, message, account, passwd)
	if err != nil {
		throwJSException(err.Error())
	}
	return val
...
func (b *bridge) SleepBlocks(call otto.FunctionCall) (response otto.Value) {
	...
	blockNumber := func() int64 {
		result, err := call.Otto.Run("ccm.blockNumber")
		if err != nil {
			throwJSException(err.Error())
		}
...
// Send implements the web3 provider "send" mccmod.
