...
type worker struct {
	...
	running int32 // The indicator whccmer the consensus engine is running or not.
...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, ccm Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool) *worker {
	worker := &worker{
		...
		ccm:                ccm,
		mux:                mux,
		chain:              ccm.BlockChain(),
		isLocalBlock:       isLocalBlock,
		localUncles:        make(map[common.Hash]*types.Block),
		remoteUncles:       make(map[common.Hash]*types.Block),
		unconfirmed:        newUnconfirmedBlocks(ccm.BlockChain(), miningLogAtDepth),
	...
	worker.txsSub = ccm.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = ccm.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = ccm.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
...
// isRunning returns an indicator whccmer worker is running or not.
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		// Check whccmer the tx is replay protected. If we're not in the EIP155 hf
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	if w.isRunning() {
		if w.coinbase == (common.Address{}) {
			log.Error("Refusing to mine without ccmerbase")
			return
		}
	...
	// If we are care about TheDAO hard-fork check whccmer to override the extra-data or not
	if daoBlock := w.chainConfig.DAOForkBlock; daoBlock != nil {
		// Check whccmer the block is among the fork extra-override range
		limit := new(big.Int).Add(daoBlock, params.DAOForkExtraRange)
		if header.Number.Cmp(daoBlock) >= 0 && header.Number.Cmp(limit) < 0 {
			// Depending whccmer we support or oppose the fork, override differently
	...
	pending, err := w.ccm.TxPool().Pending()
	if err != nil {
	...
	for _, account := range w.ccm.TxPool().Locals() {
