...
func main() {
	...
	// Pre-generate the ccmash mining DAG so we don't race
	ccmash.MakeDataset(1, filepath.Join(os.Getenv("HOME"), ".ccmash"))
	...
	for _, node := range nodes {
		...
		if err := ccmchain.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		if err := ccmchain.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := ccmchain.TxPool().Stats(); pend > 2048 {
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		return ccm.New(ctx, &ccm.Config{
			...
			GPO:             ccm.DefaultConfig.GPO,
			Ethash:          ccm.DefaultConfig.Ethash,
			Miner: Config{
