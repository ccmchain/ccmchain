...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-ccm/master/doc/ccmapp.asc
...
type ledgerDriver struct {
	...
	browser bool          // Flag whccmer the Ledger is in browser mode (reply channel mismatch)
...
// offline returns whccmer the wallet and the Ethereum app is offline or not.
//
// The mccmod assumes that the state lock is held!
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whccmer legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
