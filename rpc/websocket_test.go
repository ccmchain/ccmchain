...
// This test checks whccmer calls exceeding the request size limit are rejected.
...
func TestClientWebsocketPing(t *testing.T) {
	...
	// This is important for reproducing https://github.com/ccmchain/go-ccmchain/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	...
	const (
		...
		subNotify = `{"jsonrpc":"2.0","mccmod":"ccm_subscription","params":{"subscription":"0x00","result":1}}`
	)
