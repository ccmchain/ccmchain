...
// as well as utility mccmods to operate on the service environment.
...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int, namespace string) (ccmdb.Database, error) {
...
func (ctx *ServiceContext) OpenDatabaseWithFreezer(name string, cache int, handles int, freezer string, namespace string) (ccmdb.Database, error) {
	if ctx.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
