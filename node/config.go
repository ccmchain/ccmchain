...
import (
	...
	"github.com/ccmchain/go-ccmchain/accounts/external"
...
type Config struct {
	...
	// registered services, instead those can use utility mccmods to create/access
...
// ExtRPCEnabled returns the indicator whccmer node enables the external
...
var isOldGccmResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	if warn, isOld := isOldGccmResource[path]; isOld {
		oldpath := ""
		if c.name() == "gccm" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGccmResourceWarning, "Using deprecated resource file %s, please move this file to the 'gccm' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-ccmchain-keystore")
		ephemeral = keydir
	}
