...
import (
	...
	"github.com/promccmeus/tsdb/fileutil"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Ensure that the AccountManager mccmod works before the node has started.
	// We rely on this in cmd/gccm.
...
// startRPC is a helper mccmod to start all the various RPC endpoint during node
...
// at the time of invocation, the mccmod immediately returns.
func (n *Node) Wait() {
	n.lock.RLock()
	if n.server == nil {
		n.lock.RUnlock()
...
// Server retrieves the currently running P2P network layer. This mccmod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string) (ccmdb.Database, error) {
	if n.config.DataDir == "" {
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, freezer, namespace string) (ccmdb.Database, error) {
	if n.config.DataDir == "" {
		return rawdb.NewMemoryDatabase(), nil
	}
