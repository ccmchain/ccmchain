...
// Set of services all wrapping the base NoopService resulting in the same mccmod
...
// mccmods can be instrumented both return value as well as event hook wise.
...
// InstrumentingWrapper is a mccmod to specialize a service constructor returning
...
// same mccmod signatures but different outer types.
...
// OneMccmodAPI is a single-mccmod API handler to be returned by test services.
type OneMccmodAPI struct {
	fun func()
}

func (api *OneMccmodAPI) TheOneMccmod() {
	if api.fun != nil {
		api.fun()
	}
}
