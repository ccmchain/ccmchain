...
// starting nodes and connecting them togccmer.
//
...
// NOTE:
// it also checks whccmer there has been recent attempt to connect the peers
...
type Node struct {
	...
	// up tracks whccmer or not the node is running
	up   bool
	upMu sync.RWMutex
}
...
func (n *Node) UnmarshalJSON(raw []byte) error {
	...
	// Ticket: https://github.com/ccmersphere/go-ccmchain/issues/1177
	node := struct {
		Config *adapters.NodeConfig `json:"config,omitempty"`
		Up     bool                 `json:"up"`
	}{}
...
type Conn struct {
	...
	// Up tracks whccmer or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whccmer both nodes are currently up
