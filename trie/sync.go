...
type request struct {
	...
	raw  bool        // Whccmer this is a raw entry (code) or a trie node
...
type Sync struct {
	database ccmdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database ccmdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togccmer
...
// Process injects a batch of retrieved trie nodes data, returning if somccming
...
func (s *Sync) Commit(dbw ccmdb.KeyValueWriter) (int, error) {
...
func (s *Sync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whccmer known or not
	type child struct {
		node  node
		depth int
	}
