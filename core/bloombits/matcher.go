...
type Matcher struct {
	...
	running uint32 // Atomic flag whccmer a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors togccmer.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them togccmer
				var orVector []byte
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somccming must arrive before we're aborting
