...
func SetupGenesisBlock(db ccmdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	// Check whccmer the genesis block is already written.
	if genesis != nil {
		hash := genesis.ToBlock(nil).Hash()
		if hash != stored {
			return genesis.Config, hash, &GenesisMismatchError{stored, hash}
...
func (g *Genesis) ToBlock(db ccmdb.Database) *types.Block {
	if db == nil {
...
func (g *Genesis) Commit(db ccmdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db ccmdb.Database) *types.Block {
...
func GenesisBlockForTesting(db ccmdb.Database, addr common.Address, balance *big.Int) *types.Block {
...
// DeveloperGenesisBlock returns the 'gccm --dev' genesis block. Note, this must
// be seeded with the
