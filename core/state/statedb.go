...
// Package state provides a caching layer atop the Ccmchain state trie.
package state
...
// StateDBs within the ccmchain protocol are used to store anything
...
// This mccmod will panic if the refund counter goes below zero
...
// Exist reports whccmer the given account address exists in the state.
// Notably this also returns true for suicided accounts.
func (self *StateDB) Exist(addr common.Address) bool {
	return self.getStateObject(addr) != nil
}

// Empty returns whccmer the state object is either non-existent
...
//
// Setting, updating & deleting state object mccmods.
//
...
func (self *StateDB) Copy() *StateDB {
	...
	for addr := range self.journal.dirties {
		// As documented [here](https://github.com/ccmchain/go-ccmchain/pull/16485#issuecomment-380438527),
		// and in the Finalise-mccmod, there is a case where an object is in the journal but not
