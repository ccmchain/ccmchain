...
type CacheConfig struct {
	...
	TrieCleanNoPrefetch bool          // Whccmer to disable heuristic state prefetching for followup blocks
	TrieDirtyLimit      int           // Memory limit (MB) at which to start flushing dirty trie nodes to disk
	TrieDirtyDisabled   bool          // Whccmer to disable trie write caching and GC altogccmer (archive node)
...
type BlockChain struct {
	...
	db     ccmdb.Database // Low level persistent database to store final content in
	...
	shouldPreserve  func(*types.Block) bool        // Function used to determine whccmer should preserve the given block.
...
// Processor.
func NewBlockChain(db ccmdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	// the head block (ccmash cache or clique voting snapshot). Might as well do
	// it in advance.
...
// empty returns an indicator whccmer the blockchain is empty.
...
// loadLastState loads the last known chain state from the database. This mccmod
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db ccmdb.KeyValueWriter, header *types.Header) {
	...
	delFn := func(db ccmdb.KeyValueWriter, hash common.Hash, num uint64) {
...
//
// This mccmod only rolls back the current block. The current header and current
...
// insert injects a new head block into the current block chain. This mccmod
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	// and returns a indicator whccmer the inserted blocks are canonical.
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			// Migrate all ancient blocks. This can happen if someone upgrades from Gccm
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			stats.processed++
			if batch.ValueSize() >= ccmdb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - ccmdb.IdealBatchSize)
			}
...
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// Sanity check that we have somccming meaningful to import
	if len(chain) == 0 {
		return 0, nil
	}
...
//
// This mccmod is split out so that import batches that require re-injecting
...
// found.
//
// The mccmod writes all (header-and-body-valid) blocks to disk, then tries to
...
//
// The verify parameter can be used to fine tune whccmer nonce verification
