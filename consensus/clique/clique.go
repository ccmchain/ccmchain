...
var (
	...
	// errInvalidVote is returned if a nonce value is somccming else that the two
...
type Clique struct {
	...
	db     ccmdb.Database       // Database to store and retrieve snapshot checkpoints
...
func New(config *params.CliqueConfig, db ccmdb.Database) *Clique {
...
// VerifyHeader checks whccmer a header conforms to the consensus rules.
func (c *Clique) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	return c.verifyHeader(chain, header, nil)
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers. The
// mccmod returns a quit channel to abort the operations and a results channel to
...
// verifyHeader checks whccmer a header conforms to the consensus rules.The
...
// VerifySeal implements consensus.Engine, checking whccmer the signature contained
// in the header satisfies the consensus protocol requirements.
func (c *Clique) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	return c.verifySeal(chain, header, nil)
}

// verifySeal checks whccmer the signature contained in the header satisfies the
// consensus protocol requirements. The mccmod accepts an optional list of parent
...
//
// Note, the mccmod requires the extra data to be at least 65 bytes, otherwise it
